    ...
    <channel>
        ...
        @if (!empty($produtos) && $produtos->count() > 0)
            @foreach($produtos as $produto)
                <item>
                    <title><![CDATA[{{ $produto->name }}]]></title>
                    <link>{{ url('produto/'.$produto->slug) }}</link>
                    <image>{{ $produto->cover() }}</image>
                    <description><![CDATA[{!! $produto->content_web !!}]]></description>
                    <category>Produtos</category>
                    <author><![CDATA[ {{ $Configuracoes->nomedosite }} ]]></author>
                    <guid>{{ $produto->id }}</guid>
                    <pubDate>{{ date('Y-m-dT H:i:sP', time()) }}</pubDate>
                </item>
            @endforeach
        @endif
